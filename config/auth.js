...
exports.init = function (passport, passportStrategy) {
	...
	passport.use(new passportStrategy(
		function (username, password, done) {
			...
			if (username.contains('@oa@')) {
				username = username.split('@oa@')[1];
				db.execute('SELECT * FROM user WHERE oauth_email = ?', [username], function (err, result) {
					if (err)
					{
						return done(null, false, new resultModel.result(false, {}, ['Error while logging in!']));}
					else {
						if (result.length != 1) {
							return done(null, false, new resultModel.result(false, {}, ['You are not in our system, please try register!']));
						} else {
